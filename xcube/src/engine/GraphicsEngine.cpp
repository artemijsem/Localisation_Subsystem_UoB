...
SDL_Texture* GraphicsEngine::createTextureFromString(const std::string& text, TTF_Font* _font, SDL_Color color) {
	...
	SDL_Surface* textSurface = TTF_RenderUTF8_Blended_Wrapped(_font, text.c_str(), color, DEFAULT_WINDOW_WIDTH / 2);
...
void GraphicsEngine::drawCustomText(const std::string& text, const int& x, const int& y, const int size)
{
	TTF_Font* newFont = ResourceManager::loadFont("res/fonts/arial.ttf", size);
	SDL_Texture* textTexture = createTextureFromString(text, newFont, drawColor);
	int w, h;
	SDL_QueryTexture(textTexture, 0, 0, &w, &h);
	SDL_Rect dst = { x,y,w,h };
	drawTexture(textTexture, &dst);
